...
.liv_xgboost_variable_types <- function(X, cts_num = 4) {
...
.liv_xgboost_family_name <- function(family) {
...
.liv_xgboost_constant_fit <- function(Y, family) {
  value <- mean(as.numeric(Y), na.rm = TRUE)
  if (!is.finite(value)) {
  ...
  list(
    type = "constant",
    value = value,
    family = .liv_xgboost_family_name(family)
  )
}

.liv_xgboost_prepare_frame <- function(data,
                                      continuous_vars,
                                      discrete_vars,
                                      levels = NULL) {
  ...
  for (var in continuous_vars) {
    ...
    data[[var]][!is.finite(data[[var]])] <- NA_real_
  }
  ...
  if (is.null(levels)) {
    levels <- lapply(discrete_vars, function(var) {
      ...
      sort(unique(c(values, "__OTHER__")))
    })
  ...
  for (var in discrete_vars) {
    ...
    values[is.na(values) | !values %in% levels[[var]]] <- "__OTHER__"
...
.liv_xgboost_matrix <- function(data,
                                continuous_vars,
                                discrete_vars,
                                levels = NULL,
                                feature_names = NULL) {
  prepared <- .liv_xgboost_prepare_frame(
    data,
...
.liv_xgboost_params <- function(family) {
  family_name <- .liv_xgboost_family_name(family)
  params <- list(
    ...
    nthread = as.integer(getOption("liv.xgboost.nthread", 1L)),
    verbosity = 0
  )

  user_params <- getOption("liv.xgboost.params", list())
...
.liv_xgboost_fit <- function(Y, X, family, cts_num = 4) {
  X <- as.data.frame(X)
  y <- as.numeric(Y)
  keep <- is.finite(y)
  if (!any(keep)) {
    stop("The response has no finite values.", call. = FALSE)
  }
  X <- X[keep, , drop = FALSE]
  y <- y[keep]

  if (length(unique(y)) == 1) {
    return(.liv_xgboost_constant_fit(y, family))
  }

  family_name <- .liv_xgboost_family_name(family)
  if (family_name == "binomial" && any(!y %in% c(0, 1))) {
    stop("Binomial xgboost responses must be coded as 0/1.", call. = FALSE)
  }

  types <- .liv_xgboost_variable_types(X, cts_num = cts_num)
  continuous_vars <- types$continuous
  discrete_vars <- types$discrete
  design <- .liv_xgboost_matrix(X, continuous_vars, discrete_vars)
  dtrain <- xgboost::xgb.DMatrix(design$matrix, label = y, missing = NA_real_)
  nrounds <- as.integer(getOption("liv.xgboost.nrounds", 200L))

  fit <- tryCatch(
    xgboost::xgb.train(
      params = .liv_xgboost_params(family),
      data = dtrain,
      nrounds = nrounds,
      verbose = 0
    ),
  ...
  list(
    ...
    continuous_vars = continuous_vars,
    discrete_vars = discrete_vars,
    discrete_levels = design$levels,
    feature_names = design$feature_names
  )
}

.liv_xgboost_predict <- function(fit, newdata) {
  newdata <- as.data.frame(newdata)

  if (fit$type == "constant") {
    pred <- rep(fit$value, nrow(newdata))
  } else {
    design <- .liv_xgboost_matrix(
      newdata,
    ...
    dnew <- xgboost::xgb.DMatrix(design$matrix, missing = NA_real_)
    pred <- tryCatch(
      as.numeric(predict(fit$object, dnew)),
...
.liv_dml_overall <- function(mydat,
  ...
  fold_results <- .liv_lapply_folds(K, function(k) {
    ...
    while (time >= 0) {
      ...
      fit_Y <- .liv_xgboost_fit(oY[train], fY[train, ], family = gaussian())
      fit_A <- .liv_xgboost_fit(oA[train], fA[train, ], family = binomial())
      fit_Z <- .liv_xgboost_fit(oZ[train], fZ[train, ], family = binomial())

      Y11 <- .liv_xgboost_predict(fit_Y, fY %>% mutate(Z = 1, A = 1))
      Y10 <- .liv_xgboost_predict(fit_Y, fY %>% mutate(Z = 1, A = 0))
      Y01 <- .liv_xgboost_predict(fit_Y, fY %>% mutate(Z = 0, A = 1))
      Y00 <- .liv_xgboost_predict(fit_Y, fY %>% mutate(Z = 0, A = 0))
      pi1 <- .liv_xgboost_predict(fit_A, fA %>% mutate(Z = 1))
      pi0 <- .liv_xgboost_predict(fit_A, fA %>% mutate(Z = 0))
      omega <- pi0
      kappa <- pi1 - pi0
      f <- .liv_xgboost_predict(fit_Z, fZ)
  ...
  fold_results <- .liv_lapply_folds(K, function(k) {
    ...
    while (time >= 0) {
      ...
      if (length(unique(oden[train])) == 1) {
        ...
        fit_den <- .liv_xgboost_fit(oden[train], fY[train, ], family = gaussian())
      }
      fit_A <- .liv_xgboost_fit(oA[train], fA[train, ], family = binomial())
      fit_Z <- .liv_xgboost_fit(oZ[train], fZ[train, ], family = binomial())
      ...
      if (is.null(fit_den)) {
        ...
        den11 <- .liv_xgboost_predict(fit_den, fY %>% mutate(Z = 1, A = 1))
        den10 <- .liv_xgboost_predict(fit_den, fY %>% mutate(Z = 1, A = 0))
        den01 <- .liv_xgboost_predict(fit_den, fY %>% mutate(Z = 0, A = 1))
        den00 <- .liv_xgboost_predict(fit_den, fY %>% mutate(Z = 0, A = 0))
      }

      pA1 <- .liv_xgboost_predict(fit_A, fA %>% mutate(Z = 1))
      pA0 <- .liv_xgboost_predict(fit_A, fA %>% mutate(Z = 0))
      f <- .liv_xgboost_predict(fit_Z, fZ)
      trt <- .liv_get_treatment(action_list, time, fY)
...
# Build an action at time t from a treatment rule. simcausal evaluates the
# distribution after L[t] and Z[t] have been generated, so the rule sees the
# covariate history under the preceding interventions. The current A[t] is
# deliberately unavailable because it is the value being assigned.
.liv_policy_node <- function(policy, time) {
  force(policy)
  force(time)
  distribution <- function(n, ...) {
    fY <- as.data.frame(list(...))
    treatment <- policy(fY)
    if ((!is.numeric(treatment) && !is.logical(treatment)) ||
        !length(treatment) %in% c(1L, n) ||
        anyNA(treatment) || any(!treatment %in% c(0, 1))) {
      stop("The treatment rule at time ", time,
           " must return one 0/1 value or one 0/1 value per person.",
           call. = FALSE)
    }
    rep(as.numeric(treatment), length.out = n)
  }

  policy_params <- list(L = "L[t]", Z = "Z[t]")
  if (time > 0L) {
    for (lag in seq_len(time)) {
      for (role in c("L", "Z", "A")) {
        policy_params[[paste0(role, "_lag", lag)]] <-
          paste0(role, "[t-", lag, "]")
      }
    }
  }
  # simcausal resolves distributions by name. Give each time point its own
  # name so an earlier node cannot accidentally use a later rule.
  distr_name <- paste0("r_liv_policy_t", time)
  assign(distr_name, distribution, envir = environment())
  node("A", t = time, distr = distr_name, params = policy_params)
}

.liv_action_nodes <- function(PROB, TM) {
  if (length(PROB) != TM + 1L) {
    stop("PROB must have one treatment value or rule for each time from 0 to TM.",
         call. = FALSE)
  }
  rules <- as.list(PROB)
  nodes <- list()
  for (time in 0:TM) {
    rule <- rules[[time + 1L]]
    if (is.function(rule)) {
      nodes <- c(nodes, .liv_policy_node(rule, time))
    } else {
      if ((!is.numeric(rule) && !is.logical(rule)) || length(rule) != 1L ||
          is.na(rule) || !rule %in% c(0, 1)) {
        stop("PROB[[", time + 1L, "]] must be 0, 1, or a function.",
             call. = FALSE)
      }
      if (rule == 1) {
        nodes <- c(nodes, node("A", t = time, distr = "rconst", const = 1))
      } else {
        nodes <- c(nodes, node("A", t = time, distr = "rconst", const = 0))
      }
    }
  }
  nodes
...
ATE_local <- function(D, TM, PROB, accuracy = 1e6, state = 1) {
  PROB <- .liv_action_nodes(PROB, TM)
  ate <- rep(0, TM + 2)
  for (s in 0:(TM + 1)) {
    if (s < (TM + 1)) {
