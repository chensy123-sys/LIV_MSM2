source("Script/par/par_complier_continuous_jtpa.R")
source("LIV_xgboost.R")

set.seed(2025)
Looptime <- 500
sample_sizes <- c(10000)
act <- list(
  function(fY) return(1),
  function(fY) {return(as.numeric(fY$L>=0.8368623))}
)

for (n in sample_sizes) {
  output_dir <- file.path("data", paste0("jtpa", n))
  dir.create(output_dir, showWarnings = FALSE)

  RES <- array(0, dim = c(TM + 2, 5, Looptime))
  for (loop in seq_len(Looptime)) {
    print(paste("n =", n, "loop =", loop))
    data_LIV <- sim_LIV_local(n, D)
    fit_LIV <- DML_LIV(mydat = data_LIV, action_list = act, estimand = "local")
    RES[, , loop] <- as.matrix(predict_LIV(fit_LIV))
  }
  save(RES, file = file.path(output_dir, "Complier_1n.RData"))
}
